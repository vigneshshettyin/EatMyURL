...
// You can also add types to the function arguments and return value
// You can also add a try catch block to handle errors
// You can also add comments to the function
// Also, below code for converting osData, browserData, countryData, cityData, deviceData can be reduced to a single function
// Create a class
...
export async function getAnalyticsAction(shortcode: string) {
  ...
  const weeklyEngagements = await axios.get(
    url + `weekly?code=${shortcode}&startdate=${startdate}&enddate=${enddate}`,
    {
      headers: {
        apikey: apiKey,
      },
  ...
  if (osData.data.length != 0) {
    for (let i = 0; i < osData.data.length; i++) {
      ...
      } else os[osData.data[i].os] = osData.data[i].record_count;
    }
  }
  ...
  if (cityData.data.length != 0) {
    for (let i = 0; i < cityData.data.length; i++) {
      if (cityData.data[i].city == "") {
        city["Others"] += cityData.data[i].record_count;
      } else city[cityData.data[i].city] = cityData.data[i].record_count;
    }
  }
  ...
  if (country.length != 0) {
    ...
    for (let i = 0; i < countryData.data.length; i++) {
      ...
      country[i]["percentage"] = (
        (Number.parseInt(country[i].record_count) / totalClicks) *
        100
      ).toFixed(2);
    }
